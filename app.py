...
from blmodel import metrics, plots
from blmodel.config import (
    CovConfig,
    CovMethod,
    DataConfig,
    DeltaMethod,
...
from blmodel.covariance import annualized_volatilities, cov_to_corr
from blmodel.data import DataError
from blmodel.io_utils import (
    build_csv,
    build_excel,
    export_config,
    import_config,
...
from blmodel.optimizer import OptimizationError, unconstrained_weights
from blmodel.pipeline import (
...
def section_summary(run) -> None:
    ...
    with left:
        st.plotly_chart(plots.plot_allocation(p.weights, theme), width="stretch")
    with right:
        st.markdown("**Pesos**")
        table = pd.DataFrame({
            "Peso": p.weights,
...
def section_data(run) -> None:
    ...
    with left:
        st.plotly_chart(
            plots.plot_correlation_heatmap(cov_to_corr(run.cov), st.session_state["theme"]),
            width="stretch",
        )
    with right:
...
def section_views(run) -> None:
    ...
    if diag is not None:
        ...
        st.plotly_chart(plots.plot_view_absorption(diag, st.session_state["theme"]),
                        width="stretch")
...
def section_posterior(run) -> None:
    ...
    st.plotly_chart(plots.plot_returns_comparison(run.prior.pi, run.bl.mu_bl, theme),
                    width="stretch")
    ...
    with right:
        st.plotly_chart(plots.plot_view_impact(run.bl.view_impact, theme), width="stretch")
...
def section_optimization(run) -> None:
    ...
    st.plotly_chart(plots.plot_weights_comparison(weights, theme), width="stretch")
    ...
    with left:
        if run.frontier is not None and not run.frontier.empty:
            marks = {
                ...
                "Mercado": (
                    metrics.portfolio_volatility(run.prior.weights, run.bl.cov_bl),
                    metrics.portfolio_return(run.prior.weights, run.bl.mu_bl),
                ),
            }
            st.plotly_chart(plots.plot_efficient_frontier(run.frontier, marks, theme),
                            width="stretch")
    with right:
        if run.risk_contrib is not None:
            st.plotly_chart(plots.plot_risk_contribution(run.risk_contrib, theme),
                            width="stretch")
...
def section_sensitivity(run) -> None:
    ...
    if sens is not None and not sens.empty:
        st.plotly_chart(plots.plot_tau_sensitivity(sens, theme), width="stretch")
    ...
    if run.config.views.method is OmegaMethod.IDZOREK and run.views.n_views > 0:
        ...
        if csens is not None and not csens.empty:
            st.plotly_chart(plots.plot_confidence_sensitivity(csens, theme), width="stretch")
